...
const annotationLabels = ["Crown Tip", "CEJ", "Bone Level", "Root Apex"];
// 1. Define two sets of labels
const paAnnotationLabels = ["Crown Tip", "CEJ", "Bone Level", "Root Apex"];
const bwAnnotationLabels = ["Tooth Axis Point", "CEJ", "Bone Level"]; // New 3-point labels
...
const ImageAnalyzer = ({ onClose, onSave, slotId, isVertical, initialData }) => {
  ...
  const [annotationPoints, setAnnotationPoints] = useState([]);
    // 2. Conditionally choose the labels based on the 'isVertical' prop
  ...
  const draw = useCallback(() => {
    ...
    if (step === ProcessingStep.KEYSTONE && originalImage) {
        ...
        img.onload = () => {
            ...
            // Draw current analysis
            if (activeTooth) {
                // Draw Tooth Axis (in progress or complete)
                ...
                // Draw annotation points and their projection lines
                if (toothAxisPoints.length === 2) {
                    annotationPoints.forEach((p, i) => {
                     ...
                     // Draw preview line
                    if (mousePos && annotationSubStep === 'PLACE_POINTS' && annotationPoints.length < 4) {
                        const projectedP = projectPointOnLine(mousePos, toothAxisPoints[0], toothAxisPoints[1]);
                        ctx.beginPath();
                        ctx.moveTo(mousePos.x, mousePos.y);
                        ctx.lineTo(projectedP.x, projectedP.y);
                        ctx.strokeStyle = pointColors[annotationPoints.length];
                        ctx.lineWidth = 1;
                        ctx.setLineDash([2, 2]);
                        ctx.stroke();
                        ctx.setLineDash([]);
  ...
  }, [originalImage, step, keystonePoints, processedImage, annotationPoints, activeTooth, completedReports, toothAxisPoints, mousePos, annotationSubStep]);
  ...
  const handleInteractionStart = (e) => {
    ...
    if (step === ProcessingStep.KEYSTONE) {
        ...
        } else if (annotationSubStep === 'PLACE_POINTS' && annotationPoints.length < 4) {
        ...
        if (event.target?.result) {
          ...
          img.onload = () => {
            ...
            if (img.width > img.height) {
              newWidth = MAX_DIM;
              newHeight = (img.height * MAX_DIM) / img.width;
            } else {
              newHeight = MAX_DIM;
              newWidth = (img.width * MAX_DIM) / img.height;
            }
  ...
  const analyzePoints = () => {
    if (annotationPoints.length < 2) return; // Needs CEJ and Bone Level
const [cejPoint, bonePoint] = annotationPoints;
const PIXELS_PER_MM = 10; // !! IMPORTANT: This is a placeholder!
const distanceInPixels = dist(cejPoint, bonePoint);
const distanceInMm = distanceInPixels / PIXELS_PER_MM;

    if (annotationPoints.length < 4 || toothAxisPoints.length < 2 || !activeTooth) return;
    ...
    if (rootLen === 0) { alert("Root length cannot be zero. Please check CEJ and Apex points."); return; }
    ...
    const newReport = { toothNumber: activeTooth, crownRootRatio, attachmentLossPercent, prognosis, axis: [axisStart, axisEnd], annotations: annotationPoints };
  ...
  const renderStepContent = () => {
    switch(step) {
        ...
        <div className="text-center">
            ...
            <div className="flex justify-center items-center gap-4">
              <button onClick={() => fileInputRef.current?.click()} className="flex flex-col items-center gap-2 p-6 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors border">
                <UploadIcon className="w-12 h-12 text-blue-600" /><span>Upload File</span>
              </button>
              ...
              <button onClick={() => alert("Camera capture coming soon!")} className="flex flex-col items-center gap-2 p-6 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors border">
                <CameraIcon className="w-12 h-12 text-blue-600" /><span>Use Camera</span>
              </button>
            </div>
          </div>
      );
      case ProcessingStep.KEYSTONE: return (
          <div>
            ...
            <div className="text-center mt-4">
              <button onClick={processKeystone} className="px-6 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded-md font-semibold transition-colors">Confirm & Crop</button>
            </div>
          </div>
      );
      case ProcessingStep.ANNOTATE:
        ...
        if (activeTooth) {
            if(annotationSubStep === 'DRAW_AXIS') instruction = `Place ${2 - toothAxisPoints.length} more point(s) to define the long axis for Tooth ${activeTooth}.`;
            else if (annotationSubStep === 'PLACE_POINTS' && annotationPoints.length < 4) {
                instruction = `Place the ${annotationLabels[annotationPoints.length]} point for Tooth ${activeTooth}.`;
            } else {
                instruction = `Analysis for Tooth ${activeTooth} complete.`
            }
        }
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-1 flex flex-col">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                    {teethInSlot.map(tooth => {
                        ...
                        if (report) {
                            if (report.prognosis === PrognosisLevel.GOOD) colorClass = 'bg-green-100 text-green-800 hover:bg-green-200';
                            else if (report.prognosis === PrognosisLevel.FAIR) colorClass = 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200';
                            else if (report.prognosis === PrognosisLevel.POOR) colorClass = 'bg-orange-100 text-orange-800 hover:bg-orange-200';
                            else if (report.prognosis === PrognosisLevel.QUESTIONABLE) colorClass = 'bg-red-100 text-red-800 hover:bg-red-200';
                        }
              ...
              {activeTooth && (
                <div className="mt-4">
                    <h4 className="font-semibold mb-2 text-gray-800">Analysis for Tooth {activeTooth}:</h4>
                        ...
                        <>
                         <ul className="space-y-2">
                            {annotationLabels.map((label, i) => (
                                <li key={label} className="flex items-center gap-2"><span className="w-4 h-4 rounded-full border border-gray-400" style={{ backgroundColor: pointColors[i] }}></span><span className={`${annotationPoints.length > i ? 'line-through text-gray-400' : 'text-gray-700'}`}>{label}</span>{annotationPoints.length === i && <span className="text-blue-600 animate-pulse font-bold">&larr; Current</span>}</li>
                            ))}
                        </ul>
                         <div className="mt-4 flex gap-2">
                             <button onClick={() => { setAnnotationPoints([]); setToothAxisPoints([]); setAnnotationSubStep('DRAW_AXIS'); }} className="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-md text-sm">Reset</button>
                             {annotationPoints.length === 4 && (
                             <button onClick={analyzePoints} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md font-semibold">Analyze T{activeTooth}</button>
                             )}
                         </div>
                        </>
                    )}
              ...
              {completedReports.length > 0 && (
                <div className="mt-4 bg-gray-50 p-3 rounded-lg flex-grow border">
                  ...
                  <div className="space-y-2 text-sm overflow-y-auto max-h-48">
                    {completedReports.sort((a,b) => a.toothNumber.localeCompare(b.toothNumber)).map(r => (
                      <div key={r.toothNumber} className="bg-white p-2 rounded border border-gray-200 shadow-sm">
                        <p className="font-bold text-base text-gray-800">T{r.toothNumber}: <span className={`font-bold ${
                            r.prognosis === PrognosisLevel.GOOD ? 'text-green-600' :
                            r.prognosis === PrognosisLevel.FAIR ? 'text-yellow-600' :
                            r.prognosis === PrognosisLevel.POOR ? 'text-orange-600' : 'text-red-600'
                          }`}>{r.prognosis}</span></p>
                        <p className="text-xs text-gray-500">C:R Ratio: {r.crownRootRatio} | CAL: {r.attachmentLossPercent}%</p>
                      </div>
                    ))}
                  </div>
                </div>
               ...
               <div className="mt-auto pt-4">
                  <button onClick={handleSave} className="w-full px-4 py-3 bg-green-600 hover:bg-green-700 text-white rounded-md font-semibold text-lg">Save All & Close</button>
              </div>
            </div>
          </div>
        );
